<template>
  ...
  <div class="confirm-page">
    <div class="input-textarea-box">
      <textarea class="textarea" placeholder="请输入转交备注" v-model="remark"></textarea>
    </div>
    ...
    <div class="footer-box">
      ...
      <div class="confirm-button" @click="submitAudit">确认</div>
    </div>
    ...
    methods: {
      submitAudit() {
        let ajaxData = {
          ...
          user_list: this.detailInfo.userList,
          remark: this.remark
        }
        this.$http.transmitAudit(ajaxData).then(res => {
          if (res.status === "ok") {
            Toast('提交成功！');
            setTimeout(() => {
              this.$router.go(-1);
